...
export default function Home() {
  ...
  const startProcessing = async (useCustom = false) => {
    ...
    if (useCustom && customFile) {
      ...
      try {
        ...
        const res = await fetch('https://omniplant-ai.onrender.com/upload', { method: 'POST', body: formData });
  ...
  const handleReset = async () => {
    try {
      await fetch('https://omniplant-ai.onrender.com/reset', { method: 'POST' });
  ...
  const fetchGraph = () => {
    fetch('https://omniplant-ai.onrender.com/graph').then(r => r.json()).then(data => {
  ...
  const fetchStats = () => { fetch('https://omniplant-ai.onrender.com/stats').then(r => r.json()).then(setStats).catch(console.error); };

  const fetchKnowledgeGraph = () => {
    fetch('https://omniplant-ai.onrender.com/knowledge-graph').then(r => r.json()).then(data => {
      const rfNodes = data.nodes.map((n: any) => ({
  ...
  const onNodeClick = useCallback((_: any, node: any) => {
    ...
    fetch(`https://omniplant-ai.onrender.com/equipment/${node.id}`).then(r => r.json()).then(setSelectedEquipment).catch(console.error);
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('https://omniplant-ai.onrender.com/chat', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ message: userMsg }) });
  ...
  const handleSearch = (q: string) => {
    ...
    searchTimeoutRef.current = setTimeout(() => {
      fetch(`https://omniplant-ai.onrender.com/search?q=${encodeURIComponent(q)}`).then(r => r.json()).then(data => { setSearchResults(data); setShowSearchResults(true); }).catch(console.error);
    }, 300);
  };

  // --- Doc Library ---
  const openDocLibrary = () => {
    fetch('https://omniplant-ai.onrender.com/documents').then(r => r.json()).then(docs => { setAllDocuments(docs); setShowDocLibrary(true); }).catch(console.error);
  };

  const openDocViewer = (docId: string) => {
    fetch(`https://omniplant-ai.onrender.com/documents/${docId}`).then(r => r.json()).then(setViewingDoc).catch(console.error);
  };
